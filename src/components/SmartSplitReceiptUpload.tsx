...
export function SmartSplitReceiptUpload({ projects = [], onReceiptProcessed, onRefreshQBExpenses }: SmartSplitReceiptUploadProps) {
  ...
  const acceptMatch = (match: MatchedExpense) => {
    ...
    // Initialize editable categories and quantities from suggested/parsed values
    ...
    match.receipt.line_items?.forEach((item, idx) => {
      ...
      initialQuantities[idx] = item.quantity;
    });
  ...
  // Helper to group line items by category (with edited quantities)
  const groupByCategory = (
    lineItems: LineItem[], 
    categories: Record<number, string>,
    quantities: Record<number, number>
  ) => {
    const groups: Record<string, { items: Array<LineItem & { editedQty: number; calculatedTotal: number }>, total: number }> = {};
    
    lineItems.forEach((item, idx) => {
      const category = categories[idx] || item.suggested_category || 'misc';
      const editedQty = quantities[idx] ?? item.quantity;
      const calculatedTotal = editedQty * item.unit_price;
      
      if (!groups[category]) {
        groups[category] = { items: [], total: 0 };
      }
      groups[category].items.push({ ...item, editedQty, calculatedTotal });
      groups[category].total += calculatedTotal;
    });
  ...
  const finalizeImport = async () => {
    ...
    try {
      ...
      // Calculate subtotal using edited quantities
      const subtotal = selectedMatch.receipt.line_items?.reduce((sum, item, idx) => {
      ...
      for (let i = 0; i < categoryKeys.length; i++) {
        ...
        const itemNotes = group.items
          .map(item => `${item.item_name} (${item.editedQty}x)`)
          .join(', ');
